...
const BoardRead = () => {
  ...
  const handleUpdate = async (replyId: any) => {
    ...
    if (user === null && response.data.replyPassword === null) {
      ...
      user !== null &&
      (user !== response.data.replyWriter ||
        response.data.replyPassword !== null)
    ) {
      alert("해당 글을 수정할 수 없습니다.");
      return;
    }
                    ...
                    if (board !== "notice" && boardData) {
                      return (
                        <p>
                          ...
                          <span>{`${boardData.boardWriteDate[0]}.${
                            boardData.boardWriteDate[1] < 10 ? "0" : ""
                          }${boardData.boardWriteDate[1]}.${
                            ...
                            {user === null ? (
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    setReplyWriterModify(e.target.value)
                                  }
                                  required
                                />
                                {/* 비회원일시 */}
                                ...
                                <div className="tawrap">
                                  <textarea
                                    ...
                                    onChange={(e) =>
                                      setReplyContentModify(e.target.value)
                                    }
                                    required
                                  ></textarea>
                        ...
                        reply.map((replyItem, index) => (
                          <li key={index}>
                            <div className="cwinfo">
                              ...
                              <span className="date">{`${
                                replyItem.replyWriteDate[0]
                              }.${replyItem.replyWriteDate[1] < 10 ? "0" : ""}${
                                replyItem.replyWriteDate[1]
                              }.${replyItem.replyWriteDate[2] < 10 ? "0" : ""}${
                                replyItem.replyWriteDate[2]
                              }`}</span>
                              <button
                                type="button"
                                className="modify"
                                onClick={() => handleUpdate(replyItem.replyId)}
                              >
                                수정
                              </button>
                              <button
                                type="button"
                                className="delete"
                                onClick={() => handleDelete(replyItem.replyId)}
                              >
                                삭제
                              </button>
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  navigate(
                                    `/board/report/write?replyId=${replyItem.replyId}`
                                  );
                                }}
                              >
                                신고
